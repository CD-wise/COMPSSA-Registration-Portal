...
<body>
    <div class="container">
        <div class="header">
            <div class="logo">
                <img src="https://your-domain.com/compssa-logo.png" alt="COMPSSA Logo" style="width: 80px; height: 80px; border-radius: 50%; object-fit: contain;" onerror="this.style.display='none'; this.parentElement.innerHTML='<div style=\\'width: 80px; height: 80px; background: linear-gradient(135deg, #16a34a, #15803d); border-radius: 50%; margin: 0 auto 15px; display: flex; align-items: center; justify-content: center; color: white; font-size: 24px; font-weight: bold;\\'>CPS</div>';">
            </div>
...
This is an automated email. Please do not reply to this message.
    ...
    // FREE OPTION 1: Using Gmail with App Password (Recommended)
    // You need to:
    ...
    // FREE OPTION 2: Using Resend (Free tier: 3,000 emails/month)
    ...
    if (process.env.RESEND_API_KEY) {
      const response = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "COMPSSA Registration <noreply@yourdomain.com>", // You need to verify your domain
          to: [to],
          subject: `🎉 Welcome to COMPSSA - Registration Confirmed for ${studentName}`,
          html: emailHtml,
          text: emailText,
    ...
    // FREE OPTION 3: Using EmailJS (Client-side, completely free)
    // This would be handled on the frontend instead of here
    // See the alternative implementation below

    // If no email service is configured, simulate sending
    console.log("⚠️ No email service configured. Email would be sent to:", to)
    console.log("📧 Subject: 🎉 Welcome to COMPSSA - Registration Confirmed for", studentName)

    return NextResponse.json({
      success: true,
      message: "Email service not configured - email simulation completed",
      emailSent: false,
    })
  } catch (error: any) {
